...
function getUserInfo() {
    $.ajax({
        ...
        error: function(a, b, c) {
            ...
            location.href = MAIN_URL+'/login';
            /*localStorage.setItem('__AUTHED', false);
            $('#main-content').html('<div class="alerts alert-error">Authentication failed</div>');*/
        }
    })
}
...
function checkRoles () {
    if (__userInfo.type == null || __userInfo.type  == undefined || !__userInfo.type) {
        $('#main_content').html('<div class="alerts alert-error">No page found or no permission.</div>'); 
        localStorage.setItem('__AUTHED', false);
    ...
    $.ajax({
        url: API_URL + "/groups/" + __userInfo.type,
        type: "get",
        beforeSend: function (xhr) {
        ...
        success: function (response) {
            ...
            permission.push('me');
            currentPage = location.href.split(MAIN_URL+'/')[1].split('/')[0].split('?')[0];
            //console.log(__userInfo);
            ...
            if (currentPage) {
                if ($.inArray(currentPage, permission) < 0) {
                    ...
                    localStorage.setItem('__AUTHED', true);
                }
            } else {
                location.href = MAIN_URL+'/'+permission[0];
            }
            if ($('#top_navbar li#modules').length + $('#top_navbar li#posts').length + $('#top_navbar li#categories').length >= 2) {
                if ($('#top_navbar li#modules').length) {
                    $('#top_navbar li#posts').remove();
                    $('#top_navbar li#categories').remove();
                }
                else if ($('#top_navbar li#posts').length) {
                    $('#top_navbar li#categories').remove();
                }
            }
            $('.menu-one-box[attr-id]').each(function () {
                tT = $(this).attr('attr-id');
                if ($.inArray(tT, permission) < 0) {
                    $(this).remove();
                }
            })
        },

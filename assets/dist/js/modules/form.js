...
var template = '<div class="preview"><div class="remove-thumb"><i class="fa fa-times"></i></div><span class="imageHolder"><img /><span class="uploaded"></span></span><div class="progressHolder"><div class="progress"></div></div><input class="img_pr_link form-control" type="text"/></div>';
...
(function ($) {
    FormGen = function (formType) {
        ...
        this.initialize = function () {
            ...
            this.getModules();
            $thisform.loadNewsSettings();

            $('[name="type"]').change(function () {
                $('.customshow').hide();
                $('.customshow.' + $(this).val()).show();
                if ($(this).val() == 'news') {
                    $('[name="cat_all"]').change(function () {
                        if ($(this).val() == 1) {
                            $('[name="cat"]').val('all');
                            $('[name="cat"]+.chosen-container').hide();
                        } else {
                            $('[name="cat"]+.chosen-container').show();
                        }
        ...
        <div class="add-form-submit center">\
            <input value="Done" type="submit" class="btn btn-primary done-upload-img">\
        </div>';
            ...
            dropbox.UploadImages({
                ...
                uploadFinished: function (i, file, response) {
                    ...
                    if (response.status == 'error') {
                        ...
                        img = response.data.replace('.//../', WEB_URL+'/');
                        $('#img_link').val(img);
                ...
                success: function (response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        if (__menu == 'tin-tuc') { // this is a fix module
                            ...
                            if (response.cat == 'all') {
                                $('[name="cat_all"][value="1"]').attr('checked', true).closest('.radio').addClass('checked');
                            } else {
                                if (typeof response.cat == 'string') {
                                    $('[name="cat"] option[value="' + response.cat + '"]').attr('selected', 'selected');
                                } else {
                                    $.each(response.cat, function (ri, rv) {
                                        $('[name="cat"] option[value="' + rv + '"]').attr('selected', 'selected');
                                    })
                                }
                                $('[name="cat"]').trigger("chosen:updated");
                            }
                        ...
                        $thisform.editor_tpl.session.setValue(text);
                        $thisform.editor_tpl.getSession().setUseWrapMode(true);
                        $thisform.editor_tpl.commands.addCommand({
                            name: 'save',
                        ...
                        $thisform.editor_js.getSession().setMode("ace/mode/javascript");
                        $thisform.editor_js.getSession().setUseWrapMode(true);
                        $thisform.editor_js.commands.addCommand({
                            name: 'save',
                            bindKey: {win: "Ctrl-S", "mac": "Cmd-S"},
                            exec: function(editor) {
                                //console.log("saving");
        ...
        this.loadNewsSettings = function (response) {
            ...
            $.ajax({
                ...
                success: function (r) {
                    ...
                    if (response != null && response != undefined && response.cat != null && response.cat != undefined) {
                        if (typeof response.cat == 'string') {
                            $('[name="cat"] option[value="' + response.cat + '"]').attr('selected', 'selected');
                        } else {
